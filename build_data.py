...
OUTLOOK_KEYWORDS = [
    # 빠르게 자라는 직업 (KEIS 중장기 + WEF 2025 보정)
    (r"(인공지능|AI|머신러닝|반도체|2차전지|배터리|바이오|로봇|클라우드|보안 ?엔지니어|데이터)", +12),
    (r"(소프트웨어|앱 ?개발|웹 ?개발|콘텐츠 ?기획|UI|UX)", +10),
    (r"(빅데이터|데이터 ?사이언티스트|데이터 ?엔지니어|핀테크|블록체인)", +12),  # WEF 2025 top growing
    (r"(간병|돌봄|요양|간호|보건|의료|재활|상담|심리|사회복지)", +8),
    (r"(에너지|친환경|환경|기후|풍력|태양광|수소|전기차|EV|자율주행)", +8),
    (r"(트레이너|코치|영양사|반려동물|애견)", +6),
    # 빠르게 줄어드는 직업 (WEF 2025 fastest declining + 한국 노동 추세)
    (r"(번역|통역|기자|편집|아나운서|성우|언어)", -4),
    (r"(텔레마케터|입력원|자료입력|타자|속기|데이터 ?입력)", -10),
    (r"(은행원|보험설계사|텔러|증권 ?사무|은행 ?창구|출납창구)", -7),  # WEF: 은행 텔러 급감
    (r"(캐셔|계산원|매장 ?계산|점원 ?계산)", -8),  # WEF: 캐셔 fastest declining
    (r"(우편 ?사무|우편 ?집배|우체국 ?사무)", -6),  # WEF: 우편사무원 감소
    (r"(그래픽 ?디자이너|시각 ?디자이너|편집 ?디자이너)", -5),  # WEF: 생성형 AI 영향
    (r"(인쇄|제본|단순|단순노무|판매원|매장|점원)", -4),
    (r"(농업|어업|임업|광부|채굴)", -6),
    (r"(섬유|봉제|제직|방적|염색)", -8),
    (r"(비서|일반 ?행정 ?보조|행정 ?보조)", -5),  # WEF: 행정보조 감소
]
...
def big_avg_pay(big: str) -> int:
    ...
    출처: 고용노동부 사업체노동력조사 2025년 상반기 발표
    - 전체 상용근로자 평균 418.8만/월 = 5,025만/년 (전년 +3.5%)
    - 17개 업종 분포: 금융·보험 805만 ~ 숙박·음식 263만/월
    """
    pay_by_big = {
        "0": 6500,  # 관리자(8800) + 사무(4500) + 금융전문가 — 가중 평균. 금융업 805만/월 반영
        "1": 6300,  # 전문가(공학·연구) — 정보통신·전기가스 731만/월 반영, +5%
        "2": 5000,  # 교육·법률·사회복지 — 교육서비스 503만/월 + 법률 1억+ 가중
        "3": 6800,  # 보건의료 — 전문의 1억+ 가중평균 (보건업 평균 422만/월 + 의사 영향)
        "4": 4400,  # 예술·디자인·방송·스포츠 — 비정기성 반영. 예술 360만/월
        "5": 2850,  # 미용·여행·음식·경비·돌봄·청소 — 숙박음식 263만/월·기타 290만 가중
        "6": 3450,  # 영업·판매·운전·운송 — 도소매 320만/월 + 운수 380만/월 가중
        "7": 4200,  # 건설·채굴 — 건설 414만/월
        "8": 4100,  # 설치·정비·생산 — 제조 442만/월
        "9": 2800,  # 농림어업
    }
...
KNOW_TO_KSCO_WEIGHTS = {
    ...
    # 손실 없도록 KSCO 별 분배 합 = 1.0 정합화
    "5": [("서비스종사자", 0.93), ("단순노무", 0.40)],  # 미용·여행·음식·경비·돌봄·청소
    "6": [("판매종사자", 1.0), ("장치기계조작", 0.42)],  # 영업·판매·운전·운송
    "7": [("기능원관련", 0.40), ("단순노무", 0.15)],  # 건설·채굴
    "8": [("기능원관련", 0.60), ("장치기계조작", 0.58), ("단순노무", 0.45)],  # 설치·정비·생산
    "9": [("농림어업", 1.0)],  # 농림어업
}
